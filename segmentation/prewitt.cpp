...
Image prewitt(const Image &image)
{
    ...
    for (int i = 1; i < image.row()-1; i++)
    {
        ...
        {
            int gx = -image(i-1, j-1) - image(i-1, j) - image(i-1, j+1) + image(i+1, j-1) + image(i+1, j) + image(i+1, j+1);
            int gy = -image(i-1, j-1) - image(i, j-1) - image(i+1, j-1) + image(i-1, j+1) + image(i, j+1) + image(i+1, j+1);
